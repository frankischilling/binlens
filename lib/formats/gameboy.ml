...
let parse limits reader =
  ...
  let add id label offset length value =
    match Parser_common.field context ~parent:"gameboy" ~id ~label ~offset ~length value with
    | None -> ()
  ...
  then (
    ...
    (match cartridge with Some value -> add "cartridge_type" "Cartridge type" 0x147L 1L (Value.enum 8 value (Parser_common.enum_name cartridge_types value)) | None -> ());
    (match rom_code with
    | Some value ->
        add "rom_size_code" "ROM size code" 0x148L 1L (Value.unsigned 8 value);
        (match rom_size (Int64.to_int value) with
        | Some size ->
            add "rom_size" "Declared ROM size" 0x148L 1L (Value.unsigned 64 size);
            if Int64.compare size (Reader.length reader) <> 0 then
              Parse_context.warning context ~code:"gameboy.rom_size_mismatch"
                ~message:"The declared Game Boy ROM size does not match the file size."
    ...
    | Some value ->
        add "ram_size_code" "RAM size code" 0x149L 1L
          (Value.enum 8 value
             (Option.map (fun size -> Int64.to_string size ^ " bytes")
                (ram_size (Int64.to_int value))))
    | None -> ());
    (match destination with Some value -> add "destination" "Destination" 0x14aL 1L (Value.enum 8 value (Some (if Int64.equal value 0L then "Japan" else "Outside Japan"))) | None -> ());
    (match version with Some value -> add "version" "Version" 0x14cL 1L (Value.unsigned 8 value) | None -> ());
    ...
    | Some stored ->
        ...
        if Int64.compare length (Int64.of_int context.Parse_context.limits.max_work_units) > 0 then
          Parse_context.warning context ~code:"gameboy.global_checksum_skipped"
            ~message:"The global checksum was skipped because the file exceeds the work budget."
            ~component:"gameboy.checksum" ()
        else
          let sum = ref 0 in
          ...
          ignore (Limits.consume_work context.Parse_context.tracker (Int64.to_int length));
          let valid = Int64.to_int stored = !sum in
          add "global_checksum" "Global checksum" 0x14eL 2L (Value.Boolean valid);
          if not valid then
            Parse_context.warning context ~code:"gameboy.global_checksum_mismatch"
...
let parser =
  { Format.id; display_name; extensions = [ ".gb"; ".gbc" ]; coverage; detect; parse }
