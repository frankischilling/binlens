...
let parse limits reader =
  ...
  let add id label offset length value =
    children :=
      match
        Parser_common.field context ~parent:"nes" ~id ~label ~offset ~length
          value
      with
      | None -> !children
  ...
  (if
     Parser_common.require_length context ~minimum:16L
   ...
   then
     ...
     if Array.for_all Option.is_some bytes then (
       ...
       in
       add "mapper" "Mapper" 6L
         (if nes2 then 3L else 2L)
