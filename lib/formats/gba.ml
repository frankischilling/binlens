...
let coverage =
  { Format.summary =
      "Game Boy Advance title, identity fields, fixed value, and header \
       checksum";
    supported =
    ...
    unsupported =
      [ "ROM payload structures"; "save-memory detection"; "emulation" ]
  }
...
let detect _limits reader =
  ...
  if logo_matches reader && fixed then
    { Format.format_id = id;
      ...
      confidence = 100;
      evidence =
        [ "Game Boy Advance logo prefix"; "fixed byte 96 at offset 00B2" ];
      contradictions = [];
...
let parse limits reader =
  ...
  if not (logo_matches reader) then
    Parse_context.error context ~code:"gba.invalid_logo"
      ~message:
        "The Nintendo logo prefix does not match a Game Boy Advance header."
