...
let coverage =
  {
    Format.summary =
      "ELF32 and ELF64 headers, program headers, section headers, and section names";
    supported =
      [
        "ELF identification";
...
let object_types =
  [ (0L, "None"); (1L, "Relocatable"); (2L, "Executable"); (3L, "Shared object"); (4L, "Core") ]
...
let add_field context children ?description ?metadata ~parent ~id ~label ~offset
    ~length value =
  match
    Parser_common.field context ?description ?metadata ~parent ~id ~label ~offset
      ~length value
  with
  | None -> children
  | Some node -> node :: children

let parse_program_headers context ~class_ ~endian ~table_offset ~entry_size ~count =
  if count = 0 then None
    ...
    if entry_size < expected then (
      ...
      | Error error ->
          Parse_context.error_from_reader context ~component:"elf.program_headers" error;
          None
      | Ok () -> (
          match
          ...
          with
          | Error error ->
              Parse_context.error_from_reader context ~component:"elf.program_headers"
                error;
              None
          | Ok table_span ->
              let entries = ref [] in
              for index = 0 to count - 1 do
                ...
                let read32 relative = Parser_common.u32 context endian (Int64.add base relative) in
                let read64 relative = Parser_common.u64 context endian (Int64.add base relative) in
                let fields, file_offset, file_size =
                  if class_ = 1 then
                    ...
                    in
                    let fields = numeric "offset" "File offset" 4L p_offset fields in
                    let fields = numeric "virtual_address" "Virtual address" 8L p_vaddr fields in
                    let fields = numeric "physical_address" "Physical address" 12L p_paddr fields in
                    let fields = numeric "file_size" "File size" 16L p_filesz fields in
                    let fields = numeric "memory_size" "Memory size" 20L p_memsz fields in
                    let fields = numeric "flags" "Flags" 24L p_flags fields in
                    let fields = numeric "alignment" "Alignment" 28L p_align fields in
                    ...
                    let fields =
                      ...
                      | None -> fields
                      | Some value ->
                          add_field context fields ~parent ~id:"flags" ~label:"Flags"
                    ...
                    in
                    let fields = numeric "offset" "File offset" 8L p_offset fields in
                    let fields = numeric "virtual_address" "Virtual address" 16L p_vaddr fields in
                    let fields = numeric "physical_address" "Physical address" 24L p_paddr fields in
                    let fields = numeric "file_size" "File size" 32L p_filesz fields in
                    let fields = numeric "memory_size" "Memory size" 40L p_memsz fields in
                    let fields = numeric "alignment" "Alignment" 48L p_align fields in
                    (fields, p_offset, p_filesz)
                in
                ...
                | _ -> ());
                let entry_span = Span.unsafe ~start:base ~length:(Int64.of_int entry_size) in
                match
                  Parse_context.node context
                    ~id:(Printf.sprintf "program_header[%d]" index) ~path:parent
                    ~label:(Printf.sprintf "Program header %d" index) ~span:entry_span
              ...
              Parse_context.node context ~id:"program_headers"
                ...
                ~span:table_span ~value:(Value.Collection (List.length !entries))
...
let parse_section_headers context ~class_ ~endian ~table_offset ~entry_size ~count
    ~string_index =
  if count = 0 then None
  else
    ...
    if entry_size < expected then (
      ...
      match Limits.consume_table_entries context.Parse_context.tracker count with
      | Error error ->
          Parse_context.error_from_reader context ~component:"elf.section_headers" error;
          None
      | Ok () -> (
          match
            Reader.table_range context.Parse_context.reader ~offset:table_offset
          ...
          with
          | Error error ->
              Parse_context.error_from_reader context ~component:"elf.section_headers"
                error;
              None
          | Ok table_span ->
              let infos = ref [] in
              for index = 0 to count - 1 do
                ...
                let flags, address, file_offset, file_size, link, info, alignment,
                    element_size =
                  if class_ = 1 then
                    ( read32 8L,
                      read32 12L,
                ...
                | Some kind, Some offset, Some length
                  when not (Int64.equal kind 8L) && not (Int64.equal length 0L) ->
                    (match Reader.range context.Parse_context.reader ~offset ~length with
                    | Ok () -> ()
                    | Error _ ->
                        Parse_context.warning context
                ...
                infos :=
                  {
                    index;
                    parent;
                    span = Span.unsafe ~start:base ~length:(Int64.of_int entry_size);
                    children = List.rev fields;
                    name_offset = name;
                    section_type;
                    file_offset;
              ...
              let string_table =
                if string_index < 0 || string_index >= count then (
                  Parse_context.warning context ~code:"elf.invalid_string_table_index"
                    ~message:"The section-name string-table index is outside the section table."
                    ~component:"elf.section_headers" ();
                  None)
                else
                  ...
                  | Some offset, Some length -> (
                      ...
                      | Error _ ->
                          Parse_context.warning context
                            ...
                            ~message:"The section-name string table points outside the file."
                            ~component:"elf.section_headers" ();
                          None)
                  | _ -> None
              in
                ...
                |> List.filter_map (fun entry ->
                       let resolved_name, name_node =
                         ...
                         | Some _, Some _ ->
                             Parse_context.warning context
                               ...
                               ~message:"A section-name offset is outside the string table."
                       ...
                       let label =
                         ...
                         | Some name when not (String.equal name "") ->
                             Printf.sprintf "Section header %d: %s" entry.index name
              ...
              Parse_context.node context ~id:"section_headers"
                ...
                ~span:table_span ~value:(Value.Collection (List.length nodes))
                ~children:nodes ())
...
let parse limits reader =
  ...
  (if Int64.compare (Reader.length reader) 4L >= 0 then
    ...
    | Some value -> add_ident "magic" "Magic" 0L 4L (Value.Bytes { summary = value; length = 4L })
    | None -> ());
  ...
  let class_ = if Int64.compare (Reader.length reader) 5L >= 0 then Parser_common.u8 context 4L else None in
  let data = if Int64.compare (Reader.length reader) 6L >= 0 then Parser_common.u8 context 5L else None in
  (match class_ with
  | Some value ->
      add_ident "class" "Class" 4L 1L
        (Value.enum 8 value
           (match value with 1L -> Some "ELF32" | 2L -> Some "ELF64" | _ -> None))
  | None -> ());
  (match data with
  | Some value ->
      add_ident "endianness" "Endianness" 5L 1L
        (Value.enum 8 value
           ...
           | _ -> None))
  | None -> ());
  let class_value = match class_ with Some (1L | 2L as value) -> Some (Int64.to_int value) | _ -> None in
  let endian =
    match data with Some 1L -> Some Endian.Little | Some 2L -> Some Endian.Big | _ -> None
  in
  ...
  | Some class_, Some endian ->
      ...
      if Parser_common.require_length context ~minimum:required
      ...
      then
        ...
        let add id label offset length value =
          children := add_field context !children ~parent:root_path ~id ~label ~offset ~length value
        in
        (match type_ with Some value -> add "type" "Object type" 16L 2L (Value.enum 16 value (Parser_common.enum_name object_types value)) | None -> ());
        (match machine with Some value -> add "machine" "Machine" 18L 2L (Value.enum 16 value (Parser_common.enum_name machines value)) | None -> ());
        (match version with Some value -> add "version" "Version" 20L 4L (Value.unsigned 32 value) | None -> ());
        let word_width = if class_ = 1 then 32 else 64 in
        let word_length = if class_ = 1 then 4L else 8L in
        (match entry with Some value -> add "entry_point" "Entry point" 24L word_length (Value.address word_width value) | None -> ());
        let program_offset_field = if class_ = 1 then 28L else 32L in
        let section_offset_field = if class_ = 1 then 32L else 40L in
        (match program_offset with Some value -> add "program_header_offset" "Program-header offset" program_offset_field word_length (Value.offset word_width value) | None -> ());
        (match section_offset with Some value -> add "section_header_offset" "Section-header offset" section_offset_field word_length (Value.offset word_width value) | None -> ());
        (match flags with Some value -> add "flags" "Flags" (if class_ = 1 then 36L else 48L) 4L (Value.unsigned 32 value) | None -> ());
        let add16 id label offset value = match value with Some value -> add id label offset 2L (Value.unsigned 16 value) | None -> () in
        add16 "header_size" "ELF header size" (if class_ = 1 then 40L else 52L) header_size;
        add16 "program_header_entry_size" "Program-header entry size" (if class_ = 1 then 42L else 54L) program_entry_size;
        add16 "program_header_count" "Program-header count" (if class_ = 1 then 44L else 56L) program_count;
        add16 "section_header_entry_size" "Section-header entry size" (if class_ = 1 then 46L else 58L) section_entry_size;
        add16 "section_header_count" "Section-header count" (if class_ = 1 then 48L else 60L) section_count;
        add16 "section_name_string_table_index" "Section-name string-table index" (if class_ = 1 then 50L else 62L) string_index;
        ...
        | Some size when Int64.compare size required < 0 ->
            Parse_context.warning context ~code:"elf.header_size_too_small"
              ~message:"The declared ELF header size is smaller than the required header."
        ...
        | Some offset, Some size, Some count ->
            ...
            in
            (match node with Some node -> children := node :: !children | None -> ())
        | _ -> ());
        (match (section_offset, section_entry_size, section_count, string_index) with
...
let parser =
  { Format.id; display_name; extensions = [ ".elf"; ".so"; ".o" ]; coverage; detect; parse }
