let option field convert = function None -> [] | Some value -> [ (field, convert value) ]
...
let value = function
  | Value.Unsigned number -> `Assoc [ ("type", `String "unsigned"); ("number", numeric number) ]
  | Signed { width; value } ->
      `Assoc
        [
          ("type", `String "signed");
  ...
  | Boolean value -> `Assoc [ ("type", `String "boolean"); ("value", `Bool value) ]
  ...
  | Address number -> `Assoc [ ("type", `String "address"); ("number", numeric number) ]
  | Offset number -> `Assoc [ ("type", `String "offset"); ("number", numeric number) ]
  | Collection count -> `Assoc [ ("type", `String "collection"); ("count", `Int count) ]
  | Null -> `Assoc [ ("type", `String "null") ]
  | Invalid message -> `Assoc [ ("type", `String "invalid"); ("message", `String message) ]

let diagnostic diagnostic =
  `Assoc
    ([
       ("severity", `String (Diagnostic.severity_to_string diagnostic.Diagnostic.severity));
...
let metadata metadata =
  `Assoc
    (option "endian" (fun endian -> `String (Endian.to_string endian)) metadata.Node.endian
    @ option "numeric_base"
        (fun base ->
          `String (match base with `Decimal -> "decimal" | `Hexadecimal -> "hexadecimal"))
...
let detection (detection : Format.detection) =
  `Assoc
    [
      ...
      ("required_minimum_length", `String (Int64.to_string detection.required_minimum_length));
      ("evidence", `List (List.map (fun value -> `String value) detection.evidence));
      ( "contradictions",
...
let document ~filename ~input_size ~detections ~selected_detection
  ...
  `Assoc
    [
      ...
      ("input", `Assoc [ ("filename", `String filename); ("size", `String (Int64.to_string input_size)) ]);
      ( "detected_format",
        match selected_detection with None -> `Null | Some value -> detection value );
      ...
      ("tree", match parse_result.root with None -> `Null | Some root -> node root);
    ]
