...
| NES | `NES 1A` | iNES and NES 2.0 header version, PRG and CHR sizes, mapper, submapper, mirroring, battery, trainer, console | Header length, size arithmetic, declared payload against file | Payload interpretation, archive containers, emulation |
| Game Boy | Nintendo logo bytes at `0104..0134` | Title, CGB flag, cartridge type, ROM and RAM size codes, destination, version, checksums | Header length, known size codes, declared ROM size, header checksum, global checksum under budget | Mapper behavior, banks, code analysis, emulation |
| Game Boy Advance | Nintendo logo prefix plus fixed byte `96` | Title, game and maker codes, fixed value, unit, device, software version, header checksum | Header length, fixed value, header checksum | Full logo comparison, payload structures, save type, code analysis, emulation |
