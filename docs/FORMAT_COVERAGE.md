...
| --- | --- | --- | --- | --- |
| ELF | `7F 45 4C 46` | `e_ident`, ELF32 and ELF64 headers, extended numbering, program and section headers, section names, symbol and dynamic entries, REL and RELA entries, notes, common DWARF section markers, selected MIPS, ARM, and RISC-V flags | Class, byte order, header length, section-zero range, entry sizes, table ranges, count and work budgets, linked sections, string offsets, note layout, segment and section file ranges | Dynamic loading, applied relocations, DWARF payloads, symbol versions and hash semantics, complete architecture flags |
| PE | MZ plus PE signature at `e_lfanew`; MZ alone is heuristic | DOS magic and `e_lfanew`, signature, COFF, PE32 and PE32+ core optional fields, checked header and section RVA mapping, export names and ordinals, import descriptors and thunks, resource metadata and payload spans, base relocations, debug records, certificates, sections | Header ranges, optional magic and size, count and work budgets, powers-of-two alignment, ambiguous or unmapped RVAs, section raw ranges, import termination, resource cycles and depth, directory record sizes | Loaded-image semantics, forwarded and delayed imports, decoded resource payloads, external debug data, Authenticode verification, CLR metadata, architecture-specific directories |
