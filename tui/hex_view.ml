...
let calculate ~input_length ~selected ~rows ~bytes_per_line =
  ...
  let selected_end =
    match Span.end_offset selected with Ok value -> value | Error _ -> selected_start
  in
  {
    first_offset;
    bytes_per_line;
...
let contains span offset =
  ...
  | Error _ -> false
  | Ok finish ->
      Int64.compare offset (Span.start span) >= 0 && Int64.compare offset finish < 0
...
let render reader selected window =
  let lines = ref [] in
  if window.selected_before then lines := "^ selected range starts above" :: !lines;
  for line = 0 to window.line_count - 1 do
    ...
    if Int64.compare offset (Reader.length reader) < 0 then
      lines := render_line reader selected offset window.bytes_per_line :: !lines
  done;
  if window.selected_after then lines := "v selected range continues below" :: !lines;
  List.rev !lines
