...
let compare ?(options = Diff.default_options) left_bytes right_bytes left right
    =
  Diff.compare ~options
    ~left_reader:(Reader.of_bytes left_bytes)
    ~right_reader:(Reader.of_bytes right_bytes)
    left right
...
let test_changed_value_and_ignore () =
  ...
  Alcotest.(check bool)
    "changed value" true
    (has_kind Diff.Changed_value differences);
  let options =
    { Diff.default_options with ignore_paths = [ "pe.coff.timestamp" ] }
  in
  let ignored = compare ~options left_bytes right_bytes left right in
  Alcotest.(check bool)
    "timestamp ignored" false
    (List.exists
       (fun difference -> String.equal difference.Diff.path "pe.coff.timestamp")
       ignored)
...
let test_added_removed () =
  ...
  Alcotest.(check bool)
    "removed node" true
    (has_kind Diff.Removed_node differences)
...
let test_raw_and_format () =
  ...
  Alcotest.(check bool)
    "raw changed" true
    (has_kind Diff.Changed_raw
       (compare ~options left_bytes right_bytes left right));
  ...
  Alcotest.run "diff"
    [ ( "diff",
        [ Alcotest.test_case "identical" `Quick test_identical;
          Alcotest.test_case "changed and ignored" `Quick
