...
let sample_tree () =
  let child =
    Node.make ~id:"field" ~path:"sample.field" ~label:"Field" ~span:(span 1L 1L)
      ~value:(Value.unsigned 8 42L) ~source_format:"sample" ()
  in
  Node.make ~id:"sample" ~path:"sample" ~label:"Sample" ~span:(span 0L 2L)
    ~value:(Value.Collection 1) ~children:[ child ] ~source_format:"sample" ()
...
let test_paths_and_order () =
  ...
  Alcotest.(check bool)
    "find" true
    (Option.is_some (Node.find_by_path root "sample.field"))
...
let test_sanitize () =
  ...
  Alcotest.(check bool)
    "renderer no escape byte" false
    (String.contains rendered '\x1b')
