...
let require_root result =
  match result.Format.root with Some root -> root | None -> Alcotest.fail "missing root"
...
let test_elf_malformed () =
  let invalid_class = Fixture_builder.corrupt_u8 (Fixture_builder.elf32_little ()) 4 9 in
  let result = parse "elf" invalid_class in
  Alcotest.(check bool) "partial" true result.Format.partial;
  let truncated = parse "elf" (Fixture_builder.truncate (Fixture_builder.elf64_little ()) 40) in
...
let test_pe_malformed () =
  ...
  Alcotest.(check bool) "bad e_lfanew partial" true (parse "pe" invalid_offset).partial;
  let missing_signature = Fixture_builder.corrupt_u8 (Fixture_builder.pe32 ()) 0x80 0 in
  Alcotest.(check bool) "missing signature partial" true (parse "pe" missing_signature).partial;
  ...
  Alcotest.(check bool) "alignment warning" true
    (List.exists (fun diagnostic -> String.equal diagnostic.Diagnostic.code "pe.invalid_alignment") result.diagnostics)
...
let test_rom_malformed () =
  ...
  Alcotest.(check bool) "GB checksum warning" true
    (List.exists (fun diagnostic -> String.equal diagnostic.Diagnostic.code "gameboy.header_checksum_mismatch") result.diagnostics);
  let gba = Fixture_builder.corrupt_u8 (Fixture_builder.gba ()) 0xb2 0 in
  let result = parse "gba" gba in
  Alcotest.(check bool) "GBA fixed warning" true
    (List.exists (fun diagnostic -> String.equal diagnostic.Diagnostic.code "gba.invalid_fixed_value") result.diagnostics);
  Alcotest.(check bool) "truncated NES" true
...
let test_detection () =
  ...
  List.iter
    (fun (expected, bytes) ->
      ...
      | best :: _ -> Alcotest.(check string) "detected" expected best.Format.format_id
...
let () =
  Alcotest.run "formats"
    [
      ( "ELF",
        [
          Alcotest.test_case "four class and endian variants" `Quick test_elf_variants;
