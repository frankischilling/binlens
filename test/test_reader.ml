...
let test_boundaries () =
  ...
  Alcotest.(check int64)
    ...
    (ok (Reader.u16 reader Endian.Little 0L));
  Alcotest.(check bool)
    "one byte beyond EOF" true
    (is_error (Reader.u16 reader Endian.Little 1L));
  Alcotest.(check bool) "offset at EOF" true (is_error (Reader.u8 reader 2L));
  Alcotest.(check bool)
    "negative offset" true
    (is_error (Reader.u8 reader (-1L)))

let test_arithmetic () =
  Alcotest.(check bool)
    "addition overflow" true
    (is_error (Reader.checked_add Int64.max_int 1L));
  Alcotest.(check bool)
    "multiplication overflow" true
    (is_error (Reader.checked_mul Int64.max_int 2L));
  Alcotest.(check int64)
    "multiply zero" 0L
    (ok (Reader.checked_mul Int64.max_int 0L));
...
let test_sub_reader_and_strings () =
  ...
  Alcotest.(check string)
    "sub bytes" "bc\000"
    (Bytes.to_string (ok (Reader.bytes sub ~offset:0L ~length:3L)));
  Alcotest.(check bool) "sub outside" true (is_error (Reader.u8 sub 3L));
  Alcotest.(check string)
    "limited C string" "abc"
    (ok (Reader.c_string reader ~offset:0L ~max_length:7));
  Alcotest.(check string)
    "C string max" "ab"
    (ok (Reader.c_string reader ~offset:0L ~max_length:2))
...
let test_signed_and_unsigned () =
  ...
  Alcotest.(check string)
    "unsigned 64 decimal" "18446744073709551615"
    (Unsigned.to_decimal unsigned);
  Alcotest.(check string)
    "unsigned 64 hex" "0xffffffffffffffff" (Unsigned.to_hex unsigned)
...
let test_span () =
  ...
  Alcotest.(check bool)
    "zero span" true
    (Span.within ~input_length:10L (ok (Span.create ~start:10L ~length:0L)));
  Alcotest.(check bool)
    "overflow end" true
    (is_error (Span.create ~start:Int64.max_int ~length:1L));
  Alcotest.(check bool)
    "contains" true
    (Span.contains span (ok (Span.create ~start:6L ~length:2L)));
  Alcotest.(check bool)
    "overlap" true
    (Span.overlaps span (ok (Span.create ~start:9L ~length:2L)));
  Alcotest.(check bool)
    "touching is not overlap" false
    (Span.overlaps span (ok (Span.create ~start:10L ~length:2L)))

let () =
  Alcotest.run "reader"
    [ ( "reader",
        [ Alcotest.test_case "integer widths and endian" `Quick
            test_integer_reads;
          Alcotest.test_case "boundaries" `Quick test_boundaries;
          Alcotest.test_case "arithmetic" `Quick test_arithmetic;
          Alcotest.test_case "sub-reader and strings" `Quick
            test_sub_reader_and_strings;
          Alcotest.test_case "signed and unsigned" `Quick
            test_signed_and_unsigned
        ] );
